...
REQUEST_TIME = Summary('hf_request_processing_seconds', 'Time spent processing request')
PROMPT_TOKEN_COUNTER = Counter('hf_prompt_token_counter', 'tokens in prompt')
GENERATED_TOKEN_COUNTER = Counter('hf_generated_token_counter', 'tokens generated by LLM')
REQUESTS_COUNTER = Counter('hf_enduser_requests_counter', 'tokens generated by LLM')

def extractGeneratedTokens(input_str):
    
    generatedPromptsOnly = input_str.split("[/INST]")[1]
    
    return generatedPromptsOnly
...
def make_request_to_llm():
  ...
  promptForTesting = "[INST]" + listOfPrompts[random_num] + "[/INST]"
  ...
  sequences = pipeline(
     promptForTesting,
      ...
      do_sample=True,
      top_k=10,
      num_return_sequences=1, #don't change this param
  ...
  generatedText = extractGeneratedTokens(sequences[0]['generated_text'])
  
  total_time = end - start 
  
  total_generated_token_count = len(tokenizer.encode(generatedText))
  total_prompt_token_count = len(tokenizer.encode(promptForTesting))
  
  print(total_generated_token_count)
  
  PROMPT_TOKEN_COUNTER.inc(total_prompt_token_count)
  GENERATED_TOKEN_COUNTER.inc(total_generated_token_count)
